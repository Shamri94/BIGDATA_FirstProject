...
hadoop fs -rm -r /user/output/tmp/tmp_Job3_TOTAL
hadoop fs -rm -r /user/output/Job3_TOTAL
hadoop \
	jar /usr/lib/hadoop-mapreduce/hadoop-streaming-2.8.5-amzn-3.jar \
	-D stream.num.map.output.key.fields=3 \
	-D mapreduce.job.reduces=1 \
	-input /user/input/historical_stock_prices_TOTAL.csv \
	-output /user/output/tmp/tmp_Job3_TOTAL \
...
hadoop \
	...
	-D stream.num.map.output.key.fields=3 \
	-D mapreduce.job.reduces=1 \
	-input /user/output/tmp/tmp_Job3_TOTAL/part-00000 \
	-output /user/output/Job3_TOTAL \
